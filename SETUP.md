...
  | Rhasspy Wake Word   |  <\-- Offline detection
...
  |   Camera Module 3   |\-------->| Camera Manager      |
...
  | Wake Word Handler  |                    | Hazard Detector    |

  | (Triggers on mic)  |                    | (Triggers on object|
...
                       | Audio Controller    |  (Piper TTS → Bluetooth)
...
*   Mic → captures user audio.
    
*   Wake Word → triggers spoken feedback or hazard checks.
    
*   Camera Module → continuously captures video frames.
    
*   Object Detector → identifies nearby obstacles.
    
*   Risk Engine → determines if alert is needed.
    
*   Audio Controller → sends TTS output to paired Bluetooth speaker.
